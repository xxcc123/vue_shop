      ...
      <el-card class="box-card">
        ...
        <tree-table class="treeTable" :data="cateList" :columns="columns"
          ...
          <template slot="isok" slot-scope="scope">
            <i class="el-icon-error" v-if="scope.row.deleted_at" style="color: red;"></i>
          ...
          <template slot="option" slot-scope="scope">
            <!-- 修改按钮 -->
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showCateEditDialog(scope.row.id)">编辑</el-button>
            <!-- 删除按钮 -->
...
export default {
  ...
  methods: {
    async getCateList() {
      const { data: res } = await this.$http.get('api/category', { params: this.queryInfo })
    ...
    async getParentCateList() {
      const { data: res } = await this.$http.get('api/category/show/', { params: { type: 2 } })
      if(res.meta.status !== 200) {
    ...
    async addCate() {
      ...
      this.$refs.addCateFormRef.validate(async valid => {
        if (!valid) return
        // 验证通过，则可以发起添加用户的网络请求
        const { data: res } = await this.$http.post('api/category', this.addCateForm)
        if(res.meta.status !== 200) {
    ...
    editCateInfo() {
      this.$refs.editCateFormRef.validate(async valid => {
        if (!valid) return
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.put('api/category/' + this.editCateForm.id, {
          cat_name: this.editCateForm.name
        })
        if (res.meta.status !== 200) {
    ...
    async removeCateById(id) {
      ...
      if(res.meta.status !== 200) {
        return this.$message.error('删除分类失败！' + res.data)
      }
